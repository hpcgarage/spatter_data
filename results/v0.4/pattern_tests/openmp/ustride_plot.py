...
# select only vector length 256 for gpu tests
    ...
    for key, grp in df.groupby(['arch']):
        ax = grp.plot(ax=ax, kind='line', x='gap', y='bw(MB/s)', label=key, color=colors[key], linewidth=4)
        print(key)
    ...
    if pctlines:
        if kern == "Gather":
            ...
            xs = [2, 1.8, 2.5, 4]
        plt.hlines(percent, xmin=xs, xmax=8, linestyles='dashed', color=['black', 'purple', 'blue', 'green'])

    ax.set_ylim(10**2*5,10**5*2)
    #ax2 = ax.twinx()
    #ax2.set_ylim(3,6)
    #ax2.set_yticks(None)
